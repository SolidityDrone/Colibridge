...
fn main() {
    ...
    let view_call_env = input.into_env().with_chain_spec(&ARB_SEPOLIA_CHAIN_SPEC);
    // Commit the block hash and number used when deriving `view_call_env` to the journal.
    env::commit_slice(&view_call_env.block_commitment().abi_encode());

    // Execute the view call; it returns the result in the type generated by the `sol!` macro.
    let returns = ViewCall::new(call, contract_address).with_caller(CALLER).execute(view_call_env);
